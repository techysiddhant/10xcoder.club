 ...
 */
export function parseValidationErrors(errors: ElysiaValidationError[]): FieldErrors {
...
export function parseDbError(error: unknown): AppError {
  ...
  if (message.includes('unique constraint') || message.includes('duplicate key')) {
    if (message.includes('url')) {
      return new AppError('CONFLICT', 'A resource with this URL already exists', 409, {
  ...
  if (message.includes('not-null constraint') || message.includes('null value in column')) {
    ...
    return new AppError('VALIDATION_ERROR', `Missing required field: ${column}`, 400, {
  ...
  return new AppError('DATABASE_ERROR', 'An unexpected database error occurred', 500)
}
...
export function parseError(error: unknown): AppError {
  ...
  const errorWithName = error as { name?: string; type?: string; status?: number }
  if (
 ...
 */
export function validationError(message: string, fieldErrors: FieldErrors): ErrorResponse {
