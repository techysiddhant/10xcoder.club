...
export class YouTubeProvider implements ScrapeProvider {
  ...
  private async scrapePlaylist(
    ...
    if (playlistVideos.length > 0) {
      ...
      try {
        ...
        const videoResult = await this.scrapeVideo(
          videoUrl,
          actualFirstVideoId,
        );
        ...
        logger.debug(
          { actualFirstVideoId, error: err },
          "Failed to fetch first video details",
        );
      }
    }
    ...
    const videoCount =
      playlist.contentDetails?.itemCount || playlistVideos.length;
    ...
    return {
      ...
      description:
        firstVideoDetails?.description || playlistSnippet.description || null,
      image:
  ...
  private async enrichPlaylistVideosWithDurations(
    ...
    const BATCH_SIZE = 50;
    const videoIdToVideo = new Map<string, PlaylistVideoItem>(
    ...
    for (let i = 0; i < videos.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        logger.debug(
          { error, batchStart: i },
          "Error fetching video durations for playlist batch",
        );
  ...
  private async fallbackScrape(
    ...
    const thumbnailVideoId =
      videoIdForThumbnail ||
      (resourceType === "youtube_video" ? playlistOrVideoId : null);
