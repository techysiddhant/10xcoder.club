...
export async function getUserVote(
  ...
  const dbVote = await db
    ...
    .from(userVote)
    .where(
      and(eq(userVote.resourceId, resourceId), eq(userVote.userId, userId)),
    )
    .limit(1);
...
export async function toggleVote(
  ...
  const newVote: VoteState =
    newVoteStr === "null" ? null : (newVoteStr as VoteType);
  const fromType: VoteType | null =
      ...
      voteLogger.error(
        {
          ...
          context:
            "BOTH queue add and Redis revert failed - manual reconciliation required",
        },
  ...
  voteLogger.info(
    { resourceId, userId, type, action, newVote },
    "Vote toggled",
  );
  ...
  if (missingIds.length > 0) {
    const dbVotes = await db
      ...
      .where(
        and(
          ...
          inArray(userVote.resourceId, missingIds),
        ),
      );
    ...
    for (const vote of dbVotes) {
      ...
      if (voteType === "upvote") {
        ...
        warmCachePipeline.sadd(
          REDIS_KEY.VOTE_DOWNVOTES(vote.resourceId),
          userId,
        );
      }
    }
