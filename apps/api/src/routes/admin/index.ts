...
export const adminRoutes = new Elysia({ prefix: '/api/admin' })
  ...
  .put(
    ...
    async ({ params, body, set }) => {
      ...
      if (!body.name && !body.label && !body.icon) {
        set.status = 400
      ...
      try {
        ...
        return {
          success: false,
          message: error instanceof Error ? error.message : 'Failed to update resource type'
        }
      }
    },
    {
  ...
  .delete(
    ...
    async ({ params, set }) => {
      try {
        ...
        // Handle foreign key constraint violation (resources exist with this type)
        const dbError = error as { code?: string }
        if (dbError.code === '23503') {
          set.status = 409
          return {
            success: false,
            message: 'Cannot delete resource type: resources exist with this type'
          }
        }
